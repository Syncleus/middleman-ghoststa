...
tagname ||= nil
title = config.ghoststa[:blog][:name]
subtitle = config.ghoststa[:blog][:description]

xml.instruct!
xml.feed "xmlns" => "http://www.w3.org/2005/Atom" do
  site_url = config.ghoststa[:blog][:url]
  ...
  xml.author { xml.name config.ghoststa[:author][:name] }
